...
data_types = {
    ...
    canopen.objectdictionary.datatypes.DOMAIN:          ("",          0),
...
def write_entry(impl: TextIO, comment: bool,
    ...
    if entry.data_type not in data_types:
        sys.exit(f"{impl.name}: index {entry.index:04x}h, subindex {entry.subindex} has "
                 f"unsupported data type 0x{entry.data_type:04x}. Missing --deftype?")
    ...
    # TODO: fill data, min, max
    impl.write(f"CANOPEN_OD_ENTRY({entry.subindex}U, 0x{entry.data_type:04x}U, "
               f"{bits}U, NULL, NULL, NULL, {size},\n"
               f"\t\t\t {attr}),\n")

def write_object_entries(impl: TextIO, obj: canopen.objectdictionary.ODVariable |
                         canopen.objectdictionary.ODArray | canopen.objectdictionary.ODRecord,
                         prefix: str) -> None:
    ...
    elif isinstance(obj, canopen.objectdictionary.ODArray | canopen.objectdictionary.ODRecord):
...
def generate_impl(cmd: str, objdict: canopen.ObjectDictionary, impl: TextIO, prefix: str) -> None:
    ...
    impl.write(f"APP_DMEM CANOPEN_OD_DEFINE({prefix}, ARRAY_SIZE({prefix}_objects), "
               f"{prefix}_objects);\n")
...
def parse_args():
    ...
    group.add_argument(
        "-d", "--deftype", dest="deftypes", action='append', default=[],
...
def main():
    ...
    for deftype in args.deftypes:
        # TODO: validate deftype string format

...
static int cmd_can_dump(const struct shell *sh, size_t argc, char **argv)
{
	...
	int std_filter_id = -ENOSPC;
	int ext_filter_id = -ENOSPC;
	int err;
	...
	if (err < 0) {
		shell_error(sh, "failed to add standard (11-bit) filter (err %d)", err);
		return err;
	}
	std_filter_id = err;
	...
	if (err < 0) {
		shell_error(sh, "failed to add extended (29-bit) filter (err %d)", err);
		can_remove_rx_filter(dev, std_filter_id);
		return err;
	}
	ext_filter_id = err;
	...
	shell_set_bypass(sh, can_shell_dump_bypass_cb, NULL);

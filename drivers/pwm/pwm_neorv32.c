				  ...
				  uint32_t period_cycles, uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	cdiv = DIV_ROUND_CLOSEST(DIV_ROUND_CLOSEST(period_cycles, prsc_tbl[prsc]), steps) - 1U;

	if ((flags & PWM_POLARITY_INVERTED) != 0U) {
		if (pulse_cycles == 0U) {
			/* Channel constant active, inverted */
			cfg = NEORV32_PWM_CFG_EN;
			duty = steps - 1U;
		} else if (pulse_cycles == period_cycles) {
			/* Channel constant inactive, inverted */
			cfg = 0U;
			duty = 0U;
		} else {
			/* Inverted PWM */
			cfg = NEORV32_PWM_CFG_EN;
			duty = DIV_ROUND_CLOSEST((uint64_t)(pulse_cycles * steps),
						 pulse_cycles - period_cycles);
		}
	} else {
		if (pulse_cycles == 0U) {
			/* Channel constant inactive, non-inverted*/
			cfg = 0U;
			duty = 0U;
		} else if (pulse_cycles == period_cycles) {
			/* Channel constant active, non-inverted */
			cfg = NEORV32_PWM_CFG_EN;
			duty = steps - 1U;
		} else {
			/* Non-inverted PWM */
	...
	cfg |= FIELD_PREP(NEORV32_PWM_CFG_PRSC, prsc) |

...
static void test_on_off(enum canopen_indicators_led_color color, const uint8_t *expected)
{
	/* Turn LED on */
...
{
	test_on_off(CANOPEN_INDICATORS_LED_COLOR_RED, rgb_color_red);
}

ZTEST(canopen_indicators_status, test_green_on_off)
{
	test_on_off(CANOPEN_INDICATORS_LED_COLOR_GREEN, rgb_color_green);
}

ZTEST(canopen_indicators_status, test_red_priority)
{
	/* Turn green LED on */
	zassert_ok(canopen_indicators_set_led_state_green(&dut, CANOPEN_INDICATORS_LED_STATE_ON));
	assert_rgb_color_set(1, rgb_color_green);

	/* Turn red LED on, red will take priority */
	zassert_ok(canopen_indicators_set_led_state_red(&dut, CANOPEN_INDICATORS_LED_STATE_ON));

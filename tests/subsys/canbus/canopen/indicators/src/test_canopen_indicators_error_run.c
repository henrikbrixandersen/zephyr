...
static void test_on_off(enum canopen_indicators_led_color color, const struct led_dt_spec *led)
{
	/* Turn LED on */
	zassert_ok(canopen_indicators_set_led_state(&dut, color, CANOPEN_INDICATORS_LED_STATE_ON));
	...
	/* Turn LED off */
	zassert_ok(canopen_indicators_set_led_state(&dut, color, CANOPEN_INDICATORS_LED_STATE_OFF));
...
{
	test_on_off(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led);
}

ZTEST(canopen_indicators_error_run, test_green_on_off)
{
	test_on_off(CANOPEN_INDICATORS_LED_COLOR_GREEN, &green_led);
}

static void test_isophase(enum canopen_indicators_led_color color, const struct led_dt_spec *led,
			  enum canopen_indicators_led_state state, uint64_t pulse_ns)
{
	const int events = 10;
	timing_t start_time;
	timing_t end_time;
	...
	zassert_ok(canopen_indicators_set_led_state(&dut, color, state));
...
{
	test_isophase(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		      CANOPEN_INDICATORS_LED_STATE_FLICKERING, PULSE_NS);
}

ZTEST(canopen_indicators_error_run, test_green_flickering)
{
	test_isophase(CANOPEN_INDICATORS_LED_COLOR_GREEN, &green_led,
		      CANOPEN_INDICATORS_LED_STATE_FLICKERING, PULSE_NS);
}
...
{
	test_isophase(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		      CANOPEN_INDICATORS_LED_STATE_BLINKING, PULSE_NS * 4U);
}
...
static void test_flash(enum canopen_indicators_led_color color, const struct led_dt_spec *led,
		       enum canopen_indicators_led_state state, uint8_t flashes)
{
...
{
	test_flash(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		   CANOPEN_INDICATORS_LED_STATE_SINGLE_FLASH, 1U);
}

ZTEST(canopen_indicators_error_run, test_green_single_flash)
{
	test_flash(CANOPEN_INDICATORS_LED_COLOR_GREEN, &green_led,
		   CANOPEN_INDICATORS_LED_STATE_SINGLE_FLASH, 1U);
}
...
{
	test_flash(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		   CANOPEN_INDICATORS_LED_STATE_DOUBLE_FLASH, 2U);
}
...
{
	test_flash(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		   CANOPEN_INDICATORS_LED_STATE_TRIPLE_FLASH, 3U);
}
...
{
	test_flash(CANOPEN_INDICATORS_LED_COLOR_RED, &red_led,
		   CANOPEN_INDICATORS_LED_STATE_QUADRUPLE_FLASH, 4U);
}
...
static void canopen_indicators_error_run_after(void *fixture)
{
	...
	/* Turn LEDs off */
	zassert_ok(canopen_indicators_set_led_state_red(&dut, CANOPEN_INDICATORS_LED_STATE_OFF));
	zassert_ok(canopen_indicators_set_led_state_green(&dut, CANOPEN_INDICATORS_LED_STATE_OFF));

	/* Allow state to propagate */
	k_sleep(K_MSEC(PROPAGATION_TIME_MS));

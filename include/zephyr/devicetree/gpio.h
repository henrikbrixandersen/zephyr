 ...
 * Example usage:
 *
 *     DT_GPIO_HOG_CTLR_BY_IDX(DT_NODELABEL(n1), 0) // DT_NODELABEL(gpio1)
 *     DT_GPIO_HOG_CTLR_BY_IDX(DT_NODELABEL(n1), 1) // DT_NODELABEL(gpio1)
 *     DT_GPIO_HOG_CTLR_BY_IDX(DT_NODELABEL(n2), 0) // DT_NODELABEL(gpio1)
 *
 * @param node_id node identifier
 ...
 *         index "idx"
 */
#define DT_GPIO_HOG_CTLR_BY_IDX(node_id, idx) \
	DT_CAT4(node_id, _GPIO_HOGS_IDX_, idx, _PH)

...
/* USB class instance state flags */
...
static struct net_buf *gs_usb_buf_alloc(struct gs_usb_data *data, const uint8_t ep)
{
...
static void gs_usb_can_tx_callback(const struct device *can_dev, int error, void *user_data)
{
	...
	/*
	 * TODO: how to get both data->pool for allocating echo host frame, data->rx_queue, and
	 * echo_id?
	 */
...
static void gs_usb_tx_thread(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		/*
		 * FIXME: we cannot pull data from the buffer here, as it will be needed in the tx
		 * callback?
		 */
		hdr = net_buf_pull_mem(buf, sizeof(*hdr));

		/* Channel already validated */
		__ASSERT_NO_MSG(hdr->channel <= data->nchannels);
...
static int gs_usb_request(struct usbd_class_data *const c_data, struct net_buf *buf, int err)
{
	...
	LOG_INF("request complete for ep 0x%02x", bi->ep);
...
static int gs_usb_preinit(const struct device *dev)
{
	...
	k_thread_create(&data->rx_thread, data->rx_stack,
			...
			gs_usb_rx_thread,
			(void *)dev, NULL, NULL,
			CONFIG_USBD_GS_USB_RX_THREAD_PRIO,
			0, K_NO_WAIT);
	...
	k_thread_create(&data->tx_thread, data->tx_stack,
			...
			gs_usb_tx_thread,
			(void *)dev, NULL, NULL,
			CONFIG_USBD_GS_USB_TX_THREAD_PRIO,
			0, K_NO_WAIT);
